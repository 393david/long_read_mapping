# Compute the accuracy for duplication
# 
...
if __name__ == "__main__":
    usage = ''' Compute the accuracy for simulated duplication.

  Usage example:
  python ''' + sys.argv[0] + ' -i mapped.sam -b duplication.groundtruth -o error_mapped.txt'
    ...
    if p < 0.0 or p > 1.0:
        ...
        sys.exit(1)
    seq_header      = []
    seq_sv_len     = {} # the duplication length in this read
    truth_pos_start = []
    truth_pos_end   = []
    truth_strand    = []
    truth_length    = []
    ...
    while line:
        ...
        truth_pos_start.append(int(aa[7]))
        truth_pos_end.append(int(aa[8]))
        truth_length.append(int(aa[-1]))
        truth_set.append(set(range(int(aa[7]), int(aa[8]) + 1)))
        seq_sv_len[aa[0]] = int(aa[-1])
        n1 += 1
    ...
    ff.close()
    print()
    print('Sequences with duplication: %d' % n1)
    num2 = 0
    covered_sv = []
    ff2 = open(sam)
    line = ff2.readline()
    ...
    for key, value in header_mapped.items():
        if key in mapped_seq_header and value == 0:
            oo.write(key + '\t' + 'Not_mapped' + '\t' + str(seq_sv_len[key]) + '\n')
        if key not in mapped_seq_header:
            oo.write(key + '\t' + 'No_mapped_output' + '\t' + str(seq_sv_len[key]) + '\n')
    oo.close()
    print('\n\n')
